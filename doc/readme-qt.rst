Coupecoin-qt: Qt4 GUI for Coupecoin
===============================
...
Alternatively, install Qt Creator and open the `Coupecoin-qt.pro` file.

An executable named `Coupecoin-qt` will be built.
...
A warning for people using the *static binary* version of Coupecoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Coupecoin is linked against libdb5.3.
...
installed causes Coupecoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
