...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Coupecoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CoupecoinTEST2\r");
            Send(hSocket, "WHO #CoupecoinTEST2\r");
        } else {
            // randomly join #Coupecoin00-#Coupecoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Coupecoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Coupecoin%02d\r", channel_number).c_str());
        }
